/**
 * Script to update Liam's profile using direct SQL queries
 * 
 * This script uses direct SQL to bypass any Prisma schema mismatches
 */
...
async function main() {
  ...
  const liamEmail = isProd ? 'liam@buildappswith.ai' : 'liam.jones@buildappswith.com';
  
  // Add extra safety check for production
  if (isProd) {
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      // Create user with minimal required fields using raw SQL
      const newUser = await prisma.$queryRaw`
        INSERT INTO "User" (id, name, email, "clerkId", "isFounder", verified, roles, "createdAt", "updatedAt")
        VALUES (
          gen_random_uuid(), 
          'Liam Jons', 
          ${liamEmail}, 
