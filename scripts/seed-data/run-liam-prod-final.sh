...
# Run the migrations to ensure required tables and columns exist
...
echo "Creating SessionType table if it doesn't exist..."
npx prisma db execute --file prisma/migrations/add_session_types_production.sql --schema prisma/schema.prisma

if [ $? -ne 0 ]; then
  echo "Error: SessionType table creation failed!"
  exit 1
fi
