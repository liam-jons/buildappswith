...
async function main() {
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      // Try with a simpler approach - just inserting required fields
      // and avoiding problematic enum fields
      const newUser = await prisma.$queryRaw`
        INSERT INTO "User" (id, name, email, "clerkId", verified, "createdAt", "updatedAt")
        VALUES (
          gen_random_uuid(), 
          'Liam Jons', 
          ${liamEmail}, 
      ...
      // Now try to update the role separately
      try {
        ...
        console.error('Warning: Could not set role. This is not critical:', roleError.message);
      }
    } else {
      userId = users[0].id;
