...
export const GET = withOptionalAuth(async (
  ...
  try {
    ...
    if (!result.success) {
      logger.warn('Invalid query parameters for GET session-types', { path, method, userId: auth?.userId, errors: result.error.flatten() });
    ...
    try {
      ...
      if (error.message === 'SessionType database implementation required') {
        logger.warn('SessionType GET using mock data - database implementation pending', { path, method, userId: auth?.userId, builderId });
    ...
    logger.error('Error in session types GET endpoint', { error: errorMessage, path, method, userId: auth?.userId });
...
export const POST = withAuth(async (
  ...
  try {
    ...
    if (!result.success) {
      logger.warn('Invalid session type data for POST session-types', { path, method, userId: auth.userId, errors: result.error.flatten() });
      return createAuthErrorResponse(
        AuthErrorType.VALIDATION, 
    ...
    const isAdminUser = auth.roles.includes(UserRole.ADMIN);
    const isOwnProfile = auth.userId === result.data.builderId;
    
    if (!isAdminUser && !isOwnProfile) {
      logger.warn('Unauthorized attempt to create session type for another builder', { path, method, userId: auth.userId, targetBuilderId: result.data.builderId });
      return createAuthErrorResponse(
    ...
    try {
      ...
      if (error.message === 'SessionType database implementation required') {
        logger.warn('SessionType POST using mock data - database implementation pending', { path, method, userId: auth.userId, data: result.data });
        const { mockSessionTypes } = await import('@/lib/scheduling/mock-data');
        const mockSessionType = { id: 'mock-' + Math.random().toString(36).substring(2, 15), ...result.data };
        const response = NextResponse.json(toStandardResponse(mockSessionType, {
          message: 'Using mock data - database implementation pending'
        }), { status: 201 });
        return addAuthPerformanceMetrics(response, startTime, true, path, method, auth.userId);
      } else {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('Error creating session type POST', { error: errorMessage, path, method, userId: auth.userId });
    Sentry.captureException(error);
    return createAuthErrorResponse(
      AuthErrorType.SERVER, 
