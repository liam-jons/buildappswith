  ...
  if (path && method) {
    ...
    if (statusCode >= 500) {
      ...
      captureException(new Error(`Auth error: ${type} - ${detail}`), {
        tags: { errorType: type, component: 'auth' },
  ...
  const response = NextResponse.json(
    {
      ...
      error: {
        type,
        detail,
        code: `AUTH_${type.replace(/[_-\s]/g, '_').toUpperCase()}`,
      }
    },
    {
      status: statusCode,
      headers: {
        'X-Error-Type': type,
        'X-Error-Code': `AUTH_${type.replace(/[_-\s]/g, '_').toUpperCase()}`,
      }
    }
  );

  // Finish the span if we started one
  if (spanId) {
    finishAuthSpan(spanId, false, detail);
  }
...
export function addAuthPerformanceMetrics(
  ...
  // Add timing header
  response.headers.set('X-Auth-Duration', `${durationFormatted}ms`);
  response.headers.set('X-Auth-Success', success ? 'true' : 'false');
  
  // Add server timing header for browser performance tracking
  response.headers.set(
    'Server-Timing', 
    `auth;dur=${durationFormatted};desc="Authentication"`
  );

  // Check for slow auth operations
  if (duration > 200) {
